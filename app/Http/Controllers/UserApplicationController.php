...
class UserApplicationController extends Controller
{
    ...
    {
       /**
        * all available applications in the company
        *
        * @var \App\Models\UserApplication $applications
        */
        $applications =  UserApplication::whereIn('internship_id', function($query) {
        ...
        return view('pages.company.application.list', ['applications'=>$applications]);
    }
    ...
    public function departmentIndex(): View
    {
        ...
        */
        ...
        return view('pages.'.$this->current_route.'.application.list', ['applications'=>$applications]);
    }
    ...
    public function filter(Request $request): View
    {
        ...
        */
        ...
        return view('pages.'.$this->current_route.'.application.filter', ['isFilterActivated'=>$isFilterActivated,'filters'=>$filters, 'internships' => $internships,'applications'=>$applications]);
    }
    ...
    public function show(int $user_application): View|RedirectResponse
    {
        ...
        if($this->checkAuthorizations(self::MODEL_APPLICATIONS, auth()->user()->type, $user_application, self::ACTION_VIEW)){
            ...
            return view('pages.'.$this->current_route.'.application.view', ['statistics' => $statistics,'user_application' => $user_application, 'internship' => $user_application->internship, 'user' => $user_application->user, 'prerequisite_responses' => $user_application->prerequisiteResponses, 'interview' => '']);
        }else{
    ...
    public function acceptApplication(int $user_application): RedirectResponse
    {
        ...
        if($this->checkAuthorizations(self::MODEL_APPLICATIONS, auth()->user()->type, $user_application, self::ACTION_ACCEPT_APPLICATION)){
            if($user_application->user->haveInternship()){
                return redirect()->route($this->current_route.'.application.view', $user_application->id)->with('error', 'User have already enrolled in another Internship!');
            }else{
                if($user_application->internship->isQuotaFull()){
                    return redirect()->route($this->current_route.'.application.view', $user_application->id)->with('error', 'Internship Quota full, no available spot!');
                }else{
                    if($user_application->internship->isEnded()){
                        return redirect()->route($this->current_route.'.application.view', $user_application->id)->with('error', 'Cannot make change to this application,Internship ended!');
                    }else{
                        if($user_application->internship->isStarted()){
                            return redirect()->route($this->current_route.'.application.view', $user_application->id)->with('error', 'Cannot make change to this application,Internship started!');
                        }else{
                            if($user_application->update(['status' => 1])){     
                                $is_mailed = sendApplicationAcceptedMail($user_application->user);
                                return redirect()->route($this->current_route.'.application.view', [$user_application->id, 'is_mailed' => $is_mailed])->with('success', 'Application status successfully changed to Accepted!');
                            }else{
                                return redirect()->route($this->current_route.'.application.view', $user_application->id)->with('error', 'Something went wrong, please try again!');
                            }
                        }
                    }
                }
            ...
            if($user_application->internship->isEnded()){
                ...
                if($user_application->internship->isStarted()){
                    ...
                    if($user_application->update(['status' => 2])){
                        return redirect()->route($this->current_route.'.application.view', $user_application->id)->with('success', 'Application status successfully changed to Rejected!');
                    }else{
                        return redirect()->route($this->current_route.'.application.view', $user_application->id)->with('error', 'Something went wrong, please try again!');
                    }
                }
            ...
            if($user_application->internship->isEnded()){
                ...
                if($user_application->internship->isStarted()){
                    ...
                    if($user_application->update(['status' => 0])){
                        ...
                        return redirect()->route($this->current_route.'.application.view', $user_application->id)->with('success', 'Application status successfully rested!');
                    }else{
                        return redirect()->route($this->current_route.'.application.view', $user_application->id)->with('error', 'Something went wrong, please try again!');
                    }
                }
