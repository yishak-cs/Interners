...
@section('header')
    @include('layout.header', ['title' => 'Company | Internship | List'])
@endsection
                                ...
                                <tbody>
                                    @foreach ($internships as $internship)
                                        <tr>
                                            ...
                                            <td>
                                                <a href="{{ route('company.department.view', $internship->department->id) }}">
                                                    {{ $internship->department->name }}
                                                </a>
                                            </td>
                                            <td>
                                                @if (!$internship->isEnded() && $internship->isStarted() && $internship->status != '2')
                                                <a href="{{ route('company.internship.start', $internship->id) }}">
                                                ...
                                                @endif
                                                <a href="{{ route('company.internship.view', $internship->id) }}">
                                                ...
                                                <a href="{{ route('company.internship.delete', $internship->id) }}" onclick="if(confirm('Are you sure, you want to End {{ $internship->title }}?') == false){event.preventDefault()}">
                                                ...
                                                @else
                                                <a href="{{ route('company.internship.delete', $internship->id) }}" onclick="if(confirm('Are you sure, you want to delete {{ $internship->title }}?') == false){event.preventDefault()}">
                                                    <button class="btn btn-danger btn-xs btn-flat">
